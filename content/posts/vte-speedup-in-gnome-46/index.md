---
title: "Just How Much Faster Are GNOME 46 Terminals?"
...
Roughly, it describes how quickly the program reacts to your actions: how much time passes from the moment you press a key on your keyboard to the pixels on your monitor changing color.
...
There are tools like [Typometer] that measure the input latency in software by detecting key press events and monitoring the screen pixels for change.
This can work reasonably well, but requires fiddling with your setup to make sure you're not introducing any biases by accident.
...
I should really get around to writing a full blog post about this latency tester, but for now you should read [this post by Tristan Hume](https://thume.ca/2020/05/20/making-a-latency-tester/) about building a similar latency tester.
I used that post as a reference for building mine, but I wrote [my own firmware](https://gist.github.com/YaLTeR/8e8bd0cddb324a9e372b32e742ff992a) and analysis scripts.

The main benefit of such a device is that it allows you to measure a full end-to-end input latency, including processing time in the kernel, the compositor, the application, then the response time of the monitor itself.
You are really measuring what you see and feel, excluding only the keyboard firmware (since the latency tester sends key press events directly over USB).
...
The board sends a keyboard key press over USB (for example, Space) and starts monitoring the light sensor readings.
As soon as it detects a jump in the light, it releases the key.
Then, it presses a second key (for example, Backspace) and waits for the light to change back.
Now we're back to square one; the firmware waits a randomized amount (to prevent aliasing with the monitor refresh rate, which happens super easily) and repeats the experiment.
...
There are 120 of these dots since I made each test take 120 measurements.

Looking at the dots can confirm that the data is sensible.
We expect the main bulk of the measurements to be spread uniformly across an interval roughly the size of one monitor refresh cycle.
This is because monitors generally repaint at a constant rate, and pressing a key at a random point in time should land us in a random point of the monitor repaint cycle.
We get the lowest latency when pressing a key causes the application to render a new frame right in time for the monitor to show it, and we get the highest latency when the application finishes rendering a new frame and *just* misses the monitor deadline, having to wait one extra repaint cycle.

High outliers in the dots, or a spread over more than one refresh cycle, indicates lag or slowness of the application under test: some key presses are taking longer than others to process.

We do not expect to see any gaps between dense dot clusters.
This would usually indicate aliasing with the monitor refresh cycle, or some frame scheduling bug in the compositor.[^1]
...
I just need to tell you what I was actually testing.

## Test Setup

I did all tests on this setup:

- [Lenovo Legion 7 Gen 7 AMD](https://www.lenovo.com/us/en/p/laptops/legion-laptops/legion-7-series/legion-7-gen-7-(16-inch-amd)/len101g0017) with AMD Ryzen 7 6800H CPU and AMD Radeon RX 6700M dGPU (using the dGPU exclusively via the MUX switch).
...
- [Console]: GTK 4, the default terminal in GNOME.[^2]
- [VTE Test App](https://gitlab.gnome.org/GNOME/vte/-/tree/0.76.0/src/app): GTK 4, a test terminal that lives in the VTE repository.
- [Terminal][GNOME Terminal]: GTK 3, used to be the default in GNOME, and is still shipped by default in several distributions.
...
The mouse cursor was outside of the window for all tests.

## Input Latency Tests

The first test is simple: I am running `cat > /dev/null` to get a place to input text (with no readline or similar processing), then I measure how long it takes for the terminal to move its block cursor one cell to the right after pressing Space.

It is meant to test the best possible scenario for the terminal, with the least overhead.
...
The light sensor alternates between an empty line and the full block landing pad.
...
Finally, let's take a closer look at all the Fedora 40 results on one plot:

![](./all-f40.png)

This plot shows how much of a toll the neovim test takes compared to a simple `cat`, but it is similar across all terminals.
...
Unfortunately, VTE still struggles on them and shows a huge spread, which pushes the rest of the results down and makes the plot less readable.

{{< inline-html >}}
<details>
<summary>Open this to see the full results if you're curious.</summary>
<img src="./vtebench-full.png" />
</details>
{{</ inline-html >}}
...
The input latency is down to almost reaching Alacritty, even in a non-trivial neovim setup with lots of complexity on screen.
...
Just make sure you're running VTE 0.76, which has all of this goodness.
...
[^2]: Your distribution may have a different idea of which terminal should be the default in its GNOME spin.
